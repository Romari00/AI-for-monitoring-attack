...
async def analyze_event(request: AnalyzeEventRequest):
    ...
    prompt = f"""
    Ты — эксперт по информационной безопасности. Дай анализ события, строго используя технический и корректный русский язык.
    ...
    **Формат ответа:**
    ...
    3. **Рекомендации**: Дай конкретные шаги по устранению угрозы.
    ...
    **Данные события**:
    ...
    return {
        "message": f"Анализ завершен для события: {request.event}",
        "response": response
    }